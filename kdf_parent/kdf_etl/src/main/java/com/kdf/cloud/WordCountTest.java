 ...
 * 
 */
public class WordCountTest extends BaseHadoop {
	...
	public static class MyMap extends Mapper<LongWritable, Text, Text, LongWritable> {
		...
		protected void map(LongWritable key, Text value, Context context) throws IOException, InterruptedException {
			...
			try {
				...
				//只查2018/11/02  且   /data/logs/nginx/error.log failed失败的数据
				if (valueStr.indexOf("2018/11/02") != -1 && valueStr.indexOf("/data/logs/nginx/error.log") != -1) {
					//记录整条错误日志
					k.set(valueStr);
	...
	/**
	 * 将Map和Reduce操作执行  测试展示error 只查2018/11/02  且   /data/logs/nginx/error.log failed失败的数据
	 */
	public static void main(String[] args) throws Exception {
		String inPath = "hdfs://192.168.31.37:9000/ly.log";// 文件路径
		String outDir = "hdfs://192.168.31.37:9000/word_count_lytest";// 输出目录，输出是建立一个目录，具体的内容在目录下
